...
class VocalSemanticCompressorV2:
    def __init__(self, sr=22050, hop_length=512):
        ...
        self.reference_guide = {
            "F0 / Pitch contour": (
                "F0는 음 높이를 나타내는 기본 주파수입니다. 구간별 평균과 표준편차는 음정 안정성, "
                "끝음 흔들림, pitch drift를 판단하는 데 씁니다. 다만 강한 반주나 잡음이 있으면 실제보다 한 옥타브 높거나 낮게 잡히는 octave error가 날 수 있습니다."
            ),
            "RMS / Energy contour": (
                "RMS는 녹음 안에서 소리 에너지가 얼마나 유지되는지 보는 값입니다. F0는 유지되는데 RMS가 떨어지면 "
                "끝음이 꺼지거나 소리 에너지 유지가 약해지는 패턴으로 볼 수 있습니다. 보정된 SPL 측정이 아니므로 "
                "절대 음압이 아니라 녹음 안에서의 상대 에너지로 해석해야 합니다."
            ),
            "HNR (Harmonics-to-Noise Ratio)": (
                "HNR은 맑은 성분에 비해 숨소리나 잡음 성분이 얼마나 섞였는지 보는 값입니다. 낮으면 숨 섞임, "
                "잡음 성분 증가, 성대 진동의 불규칙함을 의심할 수 있습니다. 다만 '성대 접촉 불량 확정'처럼 단정하면 안 됩니다."
            ),
            "Jitter / Shimmer": (
                "Jitter는 성대 진동 주기가 얼마나 미세하게 흔들리는지 보는 값이고, shimmer는 소리 크기가 얼마나 미세하게 흔들리는지 보는 값입니다. "
                "지속모음 분석에서는 의미가 있지만, 노래 전체 파일에서는 전역 참고값으로만 써야 합니다. 현재 코드는 구간별 jitter/shimmer를 계산하지 않으므로 "
                "특정 시간대에서 변했다고 말하면 안 됩니다."
            ),
            "Vibrato": (
                "비브라토 속도는 음 높이가 주기적으로 흔들리는 장식 패턴을 보는 참고값입니다. 현재 코드는 정밀한 vibrato extent, regularity, "
                "onset timing을 따로 분석하지 않으므로 과하게 해석하면 안 됩니다."
            ),
            "Spectral Centroid": (
                "Spectral centroid는 소리의 밝기 지표입니다. 높으면 밝고 날카롭게, 낮으면 어둡고 무겁게 들릴 가능성이 있습니다. "
                "단독으로 twang, belt, singer's formant를 확정하지 않습니다."
            ),
            "Formants (F1/F2/F3)": (
                "F1/F2/F3는 포먼트, 즉 모음과 입안 공명에 따라 강해지는 주파수대입니다. F1은 모음이 얼마나 열려 들리는지, "
                "F2는 모음이 앞쪽 또는 뒤쪽으로 들리는 경향, F3는 명료도와 고음역대 공명 단서로 조심스럽게 사용합니다. "
                "실제 턱, 혀, 후두 위치를 직접 측정한 값처럼 단정하면 안 됩니다."
            )
        }
